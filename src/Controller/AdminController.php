...
final class AdminController extends AbstractController
{
    ...
    {
        if($request->isMethod('POST')){if(!$this->isCsrfTokenValid('featured_save',(string)$request->request->get('_token')))throw $this->createAccessDeniedException();$position=max(1,min(10,(int)$request->request->get('position',1)));$publication=$pubs->find((int)$request->request->get('publication_id'));if($publication&&$publication->getStatut()===PublicationStatus::PUBLIE){foreach($featured->findBy(['position'=>$position]) as $old)$em->remove($old);foreach($featured->findBy(['publication'=>$publication]) as $old)$em->remove($old);$em->flush();$item=(new FeaturedPublication())->setPublication($publication)->setPosition($position);$em->persist($item);$em->flush();}return $this->redirectToRoute('app_admin_featured');}
