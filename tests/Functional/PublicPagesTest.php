...
{
    public function testHomepageHasEditorialSearchAndNavigation():void{$client=static::createClient();$client->request('GET','/');self::assertResponseIsSuccessful();self::assertSelectorTextContains('h1','Des travaux');self::assertSelectorExists('#recherche');self::assertSelectorExists('.mega');self::assertSelectorExists('.explore-chevron[aria-hidden="true"]');self::assertSelectorCount(5,'.mega > div:nth-child(2) .menu-icon[aria-hidden="true"]');}
